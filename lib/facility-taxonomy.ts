...
// 【法令台帳一覧】のmatchキーワードについて: 道路＞橋梁・トンネル以外は、当初
// 実データが無かったため長らくmatch未設定（＝検索画面では「準備中」表示、
// 検索しても常に0件）のままだった。ところが台帳（画像）の登録フォーム
// （components/FacilityLedgerForm.tsx）は元々この分類の全項目から自由に選んで
// 登録できる作りだったため、「準備中」表示の項目にも実際に登録が行われるように
// なり、「登録したのに検索で見つからない・編集しても直らない」という実害のある
// 不具合になっていた（会話ログ「片江地区や七類港が法令台帳と記載されているのに
// なぜ法令台帳に分類されていないのか」参照。実際には登録時点でdocClass="LEGAL"
// 自体は正しく保存されており、原因はここのmatch未設定だった）。登録フォームが
// 元から全項目を受け付けている以上、検索側だけ一部を「準備中」のままにしておく
// 理由が無いため、全項目にmatchを設定する。
export const FACILITY_TYPES: Record<FieldKey, FacilityTypeDef[]> = {
  road: [{ label: "道路共通", match: ["道路共通"] }, { label: "橋梁", match: ["橋"] }, { label: "トンネル", match: ["トンネル"] }],
  river_coast: [
    { label: "河川共通", match: ["河川共通"] },
    { label: "河川管理施設", match: ["河川管理施設"] },
    { label: "海岸共通", match: ["海岸共通"] },
  ],
  // 「海岸共通」「海岸保全施設」は河川・海岸だけでなく港湾にも存在する
  // （Shimaneの表では港湾分野に「海岸保全区域台帳」「海岸保全施設調書」という
  // 別の台帳様式として載っている。会話ログの指摘どおり）。
  port: [
    { label: "港湾共通", match: ["港湾共通"] },
    { label: "港湾施設", match: ["港湾施設"] },
    { label: "海岸共通", match: ["海岸共通"] },
    { label: "海岸保全施設", match: ["海岸保全施設"] },
  ],
  sabo: [
    { label: "砂防河川共通", match: ["砂防河川共通"] },
    { label: "砂防指定地台帳", match: ["砂防指定地台帳"] },
    { label: "地すべり防止区域", match: ["地すべり防止区域"] },
  ],
  park: [{ label: "都市公園", match: ["都市公園"] }],
};
