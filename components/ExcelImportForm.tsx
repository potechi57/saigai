...
export default function ExcelImportForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!(file instanceof File) || file.size === 0 || file.size <= DIRECT_UPLOAD_THRESHOLD_BYTES) {
      ...
      // （ローカル開発などVercel Blob未設定の環境でもここは動く）。
      return;
    }
  ...
  return (
    <div className="space-y-4">
      <form action={formAction} onSubmit={handleSubmit} className="space-y-4 rounded border border-gray-300 bg-white p-4">
        ...
        >
          {uploading ? "アップロード中..." : isPending ? "取り込み中..." : "取り込む"}
        </button>
      </form>
