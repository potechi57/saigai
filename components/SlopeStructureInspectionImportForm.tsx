...
import {
  importSlopeStructureInspectionExcel,
  importSlopeStructureInspectionExcelFromBlob,
} from "@/lib/actions/slope-structure-inspection-actions";
import { DIRECT_UPLOAD_THRESHOLD_BYTES, uploadFileToBlob } from "@/lib/client-blob-upload";

// 法面構造物点検調書Excelの取込フォーム（components/BridgeInspectionImportForm.tsx
// と同じ方針。1ファイル＝1箇所の詳細報告書のため、複数ファイルをまとめて選んで
// 1件ずつ順番に取り込めるようにしている）。

type FileStatus = "waiting" | "processing" | "success" | "error";

type FileState = {
  // BridgeInspectionImportForm.tsxのlistIdと同じ理由（配列indexではなく
...
async function runImport(file: File): ReturnType<typeof importSlopeStructureInspectionExcel> {
  if (file.size <= DIRECT_UPLOAD_THRESHOLD_BYTES) {
    const fd = new FormData();
    fd.append("file", file);
    return importSlopeStructureInspectionExcel(null, fd);
  }
  try {
    const blobUrl = await uploadFileToBlob(file, "slope-structure-inspection-imports");
    return await importSlopeStructureInspectionExcelFromBlob(blobUrl, file.name);
  } catch {
...
export default function SlopeStructureInspectionImportForm() {
  ...
  // （BridgeInspectionImportForm.tsxのremoveQueuedFileと同じ方針）。
  ...
  return (
    <div className="space-y-3 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
      <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">法面構造物点検調書Excelを取り込む</h2>
      <p className="text-xs text-gray-500 dark:text-gray-400">
        複数のExcelファイルを選択すると、1件ずつ順番に取り込みます。同じ管理番号（箇所番号）のファイルを再度取り込むと、内容を差し替えます（写真も含めて作り直します）。途中で1件失敗しても他のファイルの取込は続行します。
      </p>
      <label className="block text-sm">
        <span className="mb-1 block text-xs text-gray-500 dark:text-gray-400">法面構造物点検調書Excelファイル（複数選択可・.xlsx）</span>
        <input
          type="file"
          accept=".xlsx"
          multiple
                    ...
                    <tr key={f.listId} className="border-t border-gray-200 dark:border-gray-700">
                      ...
                      <td className="px-3 py-2 text-gray-600 dark:text-gray-300">
                        {f.status === "success" && f.id ? (
                          <Link href={`/inspections/slopes/${f.id}`} className="text-blue-600 dark:text-blue-400 hover:underline">
                            {f.detail} →
                          </Link>
                        ) : (
                          f.detail
