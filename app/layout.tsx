...
import { BulkImportProvider } from "@/components/BulkImportContext";
import BulkImportStatusBadge from "@/components/BulkImportStatusBadge";
          ...
          <header className="relative z-[2000] flex h-14 shrink-0 items-center justify-between border-b border-gray-300 bg-white px-6 dark:border-gray-700 dark:bg-gray-900">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 sm:gap-4">
              {/* sm未満（スマホ幅）ではPC向けの補助的なリンク群を隠す。h-14固定の
                  ...
                  BulkImportStatusBadgeは画面幅を問わず常に見えるようにしている
                  （まとめて取込中かどうかは、スマホ・PCどちらから見ていても
                  分かるようにしたいため。ハンバーガーメニューの中に隠さない）。 */}
              ...
              <nav className="hidden items-center gap-4 sm:flex">
                ...
                <Link href="/settings" className="hover:text-gray-900 hover:underline dark:hover:text-gray-100">
                  ⚙️ 設定
                </Link>
                <Link
                  href="/import"
