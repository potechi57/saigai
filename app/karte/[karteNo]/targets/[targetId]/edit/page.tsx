...
export default async function EditInspectionTargetPage({
  ...
  return (
    <div className="space-y-4">
      <Link href={`/karte/${karteNo}`} className="text-sm text-blue-600 hover:underline">
        ← カルテ詳細に戻る
      </Link>
      ...
      <section className="overflow-x-auto rounded border border-gray-400 bg-white">
        ...
        <div className="grid grid-cols-1 divide-y divide-gray-400 border-t border-gray-400 md:grid-cols-2 md:divide-x md:divide-y-0">
          <div className="p-3">
            <h2 className="mb-1 text-xs font-medium text-gray-500">
              &lt;詳細スケッチ欄&gt;・&lt;写真張付欄&gt;
            </h2>
            ...
            {target.photos.length > 0 ? (
              ...
              <p className="mb-2 text-sm text-gray-400">未登録</p>
            )}
      ...
      <div className="rounded border border-gray-300 bg-white p-4">
        <h2 className="mb-2 font-semibold text-gray-700">状態</h2>
        <p className="mb-3 text-sm text-gray-500">
          現在の状態:{" "}
          {target.isActive ? (
            <span className="text-green-700">追跡中</span>
          ) : (
            <span className="text-gray-400">解消済み（非表示）</span>
          )}
        </p>
        {target.isActive ? (
...
function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="border border-gray-400 bg-gray-100 px-2 py-1 text-left align-middle font-medium whitespace-nowrap text-gray-600">
      {children}
    </th>
  );
}
...
function Td({ children }: { children: React.ReactNode }) {
  return <td className="border border-gray-400 bg-white px-2 py-1 align-middle">{children}</td>;
}
