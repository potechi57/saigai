              ...
              {formAPhotos.length > 0 ? (
                <PhotoLightboxGroup photos={formAPhotos}>
                  {/* 「横並びを絶対に崩さない」という要件のため、折り返し（flex-wrap）も
                      ...
                      ようにした。先頭（合成画像。点検地点位置図欄）は現況写真より
                      大きく見えるようにしている。
                      【比率を1.5→3.5に引き上げた理由】合成画像は横長（点検地点位置図欄の
                      アスペクト比）になりがちで、現況写真（普通の縦横比の写真）と違い、
                      横幅を増やしても高さはあまり伸びない。1.5倍の割り当てでは、
                  ...
                  <div className="flex gap-3">
                    {formAPhotos.map((p, i) => (
                      <PhotoLightboxThumbnail
                        key={p.id}
                        index={i}
                        className={`block min-w-0 ${i === 0 ? "flex-[3.5]" : "flex-1"}`}
                      >
