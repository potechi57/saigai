...
// 検索画面（指示書6章）とカルテ一覧画面（8章）を1画面に統合している。
// 指示書19章「これらを16個の完全に独立したページとして実装する必要はない」の方針に沿い、
// 検索結果＝一覧そのもの、という自然な形にした（クエリパラメータ化しているのでURL共有も可能）。
export default async function KarteListPage({
  ...
  if (params.routeName) {
    where.routeName = { contains: params.routeName, mode: "insensitive" };
  }
  if (params.routeNo) {
