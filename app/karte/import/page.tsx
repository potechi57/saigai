    ...
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      ...
      <div className="rounded border border-yellow-300 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950 p-3 text-xs text-yellow-800 dark:text-yellow-300">
        ...
        <ul className="mt-1 list-disc pl-4">
          ...
          <li>規制基準等（連続雨量・時間雨量）は、カルテ詳細画面には表示されるようになりましたが、Excelからの自動取込は値セルの位置が実データで確認できていないため未対応です。カルテ編集画面からの入力欄もまだ無いため、現状は値を設定する手段がありません。</li>
          ...
          <li>様式Ａ・様式Ｂの写真、および「現状記録写真」（「R7現状記録写真」等の別シート。複数枚ある場合の連番シートも含む）の写真は自動で取り込みます。ただし「現状記録写真」は画像のみの取込で、各写真に添えられたキャプション文字列（「起点側全景」等）までは取り込みません。</li>
          <li>
            スケッチ等のEMF/WMF形式（ベクター画像）は、Aspose
            Cloudの認証情報（環境変数）を設定した場合のみPNGに変換して取り込みます。未設定の環境では従来どおり対象外です。行政データを外部クラウドサービスへ送信することについては、導入前に組織のセキュリティ担当部署にご確認ください（Aspose自身が第三者セキュリティ認証（SOC
            2・ISO 27001等）を取得していないことを公表しています）。
          </li>
        </ul>
      </div>
    </div>
