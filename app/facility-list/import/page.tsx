...
// 「施設一覧」形式のExcel（Accessの施設管理データベースから出力される、
// 道路法面施設等の台帳一覧）を取り込む画面。防災カルテのExcel取込
// （/karte/import）とは別の画面にしている（形式も目的も別のデータのため）。
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 p-6">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">
        島根県の点検業務で貸与される「施設一覧」形式のExcel（Accessの施設管理データベースから出力されたと思われる、道路法面施設等の一覧表）を取り込みます。管理番号ごとにDBへ登録し、緯度経度が入っている行は地図（検索・一覧画面）にもピンで表示されます。同じ管理番号の行は、再度取り込むと内容が上書き更新されます。
      </p>
      ...
      <div className="rounded border border-yellow-300 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950 p-3 text-xs text-yellow-800 dark:text-yellow-300">
        ...
        <ul className="mt-1 list-disc pl-4">
          <li>「施設一覧(道路法面施設).xlsx」の実データで確認した列位置を前提にしています。管理番号・施設分野・路線名・所在地・緯度経度・健全度・点検実施日等、想定の列位置に無いレイアウトのファイルは正しく取り込めません。</li>
