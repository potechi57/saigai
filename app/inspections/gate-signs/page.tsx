  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">点検調書（門型標識）一覧</h1>
        <Link
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">
        「別紙２　様式１様式２」形式の門型標識点検調書Excelを取り込んだ一覧です。管理番号で施設台帳（道路標識）と紐付いているものは、そちらの詳細ページへのリンクも表示します。
      </p>
      ...
      {inspections.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {inspections.map((insp) => {
            const cover = insp.overviewPhotos[0];
            ...
            return (
              <li key={insp.id} className="overflow-hidden rounded border border-gray-300 bg-white dark:border-gray-700 dark:bg-gray-900">
                <Link href={`/inspections/gate-signs/${insp.id}`}>
                  {cover ? (
                ...
                <div className="space-y-1 p-3 text-sm">
                  ...
                  <p className="font-semibold text-gray-800 dark:text-gray-100">
                    <Link href={`/inspections/gate-signs/${insp.id}`} className="hover:underline">
                      {title}
                    </Link>
                  </p>
                  ...
                  <form action={deleteGateSignInspection.bind(null, insp.id)} className="pt-1">
                    <ConfirmSubmitButton
