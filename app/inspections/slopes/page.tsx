...
// 点検調書（道路＞法面構造物）の一覧画面（app/inspections/bridges/page.tsxと同じ構成）。
// 施設台帳（FacilityListItem）とは別テーブルだが、管理番号（箇所番号）で
// 紐付いている場合はリンクを出す。
export default async function SlopeStructureInspectionListPage() {
  // supersededByInspection: null＝年度別履歴チェーンのうち現在有効な最新
  // レコードのみを一覧に出す（GateSignInspection/BridgeInspectionと同じ方式）。
  const inspections = await prisma.slopeStructureInspection.findMany({
    where: { supersededByInspection: { is: null } },
    orderBy: { createdAt: "desc" },
    include: {
      photos: { where: { category: "overview" }, orderBy: { sortOrder: "asc" }, take: 1 },
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">点検調書（法面構造物）一覧</h1>
        <Link
          href="/inspections/slopes/import"
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">
        法面構造物（モルタル吹付け・のり枠・ロックネット・擁壁・石/ブロック積擁壁・落石防護柵・雪崩防止柵）の点検調書Excelを取り込んだ一覧です。管理番号（箇所番号）で施設台帳と紐付いているものは、そちらの詳細ページへのリンクも表示します。
      </p>
          ...
          {inspections.map((insp) => {
            ...
            return (
              <li key={insp.id} className="overflow-hidden rounded border border-gray-300 bg-white dark:border-gray-700 dark:bg-gray-900">
                <Link href={`/inspections/slopes/${insp.id}`}>
                  {cover ? (
                ...
                <div className="space-y-1 p-3 text-sm">
                  ...
                  <p className="font-semibold text-gray-800 dark:text-gray-100">
                    <Link href={`/inspections/slopes/${insp.id}`} className="hover:underline">
                      {title}
                    </Link>
                  </p>
                  ...
                  <form action={deleteSlopeStructureInspection.bind(null, insp.id)} className="pt-1">
                    <ConfirmSubmitButton
